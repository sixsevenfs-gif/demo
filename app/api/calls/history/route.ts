...
export async function POST(req: NextRequest) {
  ...
  await prisma.$transaction([
    prisma.lead.update({ where: { id: call.leadId }, data: { assignedToId: call.executiveId, status: "CALL_PENDING", callingAssignmentPending: true, adminCallbackNote: note.trim(), adminCallbackAt: new Date(), adminCallbackSeenAt: null, adminCallbackSourceCallId: call.id, nextFollowUpDate: new Date() } }),
