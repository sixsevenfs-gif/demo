...
export async function GET(req: NextRequest) {
  try {
    ...
    const callsRange = params.get("callsRange") || "today";
    const callsDate = params.get("callsDate") || "";
    if (!["today", "yesterday", "lifetime", "date"].includes(callsRange) || (callsRange === "date" && (!/^\d{4}-\d{2}-\d{2}$/.test(callsDate) || Number.isNaN(new Date(`${callsDate}T00:00:00+05:30`).getTime())))) {
      return NextResponse.json({ error: "Choose a valid call date" }, { status: 400 });
    }
